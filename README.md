...
- I assigned the body element the Poppins font for a clean, modern look and set a background color of #009DDB, with flexbox alignment to center all content vertically and horizontally. For the .main-title, I used a large font size and bold weight to make it stand out, coloring it #1C6147 for visibility. The .sub-title and .sub-section-title styles adjust the font size and color for secondary headings, creating a hierarchy of text. The .container1 utilizes a grid layout to arrange the boxes in a 2x2 pattern, ensuring equal spacing between them. Each .box has a rounded background with a #00DB87 color and a subtle hover effect that moves the box up slightly, adding a dynamic touch. Inside these boxes, I use .bold-text for emphasis. For the table in one of the boxes, I defined borders around it and its cells to make the data easy to read. The footer has centered content with a dark background. I also included a .footer-image class to size the logo next to the GitHub link appropriately. The .hidden class is used to toggle the visibility of additional information sections, while the #toggle-info-button style applies to the button that shows or hides this extra content. It changes appearance on hover to provide user feedback.

- Inside the <head>, I included metadata and linked to external resources such as the Google Fonts and CSS stylesheet for styling. In the <body>, I placed the main content of the page. The <h1> tag represents the primary heading, styled to stand out prominently. I used <div> elements to group and layout content into containers. The <p> tag is used for paragraphs of text. I used the <ul> and <li> tags create bulleted lists for content within the boxes. For tabular data, I used <table>, <tr>, <th>, and <td> to structure and display the table rows and cells. The <button> tag creates an interactive button for toggling additional content visibility. Finally, the <footer> tag defines the footer section of the page, including a hyperlink (<a>) to the GitHub repository and an <img> tag for displaying an image next to the link.

### Design Achievements
- Created a color scheme using Adobe color. I wanted to pick colors close to primary and therefore I used a split complementary color scheme to make my selection. I changed the colors a bit to suit the ones I envisioned. 
- I used the Poppins google font as its minimalistic, yet it can be made colorful and still look good to be used within my context. It is popular and easily read, and it also was developed as a Devangari and therefore has its roots in Indian typefaces. As an Indian, I had to find one that represented me well. 
